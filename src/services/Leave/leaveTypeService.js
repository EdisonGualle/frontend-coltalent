...
const updateLeaveType = async (leaveTypeId, updateLeaveType) => {
    try {
        const response = await AxiosInstance.put(`leaves/types/${leaveTypeId}`, updateLeaveType);
        return response.data;
    } catch (error) {
        const customError = new Error(JSON.stringify(error.response.data));
        throw customError;
...
export { getLeaveTypes, createLeaveType, getLeaveType, updateLeaveType, deleteLeaveType };
